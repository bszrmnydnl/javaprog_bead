...
public class EditEntryDialog extends JDialog {
	...
	public EditEntryDialog(JFrame parent, int row, BookTableModel tableModel) throws ClassNotFoundException {
		...
		tfId.setText(String.valueOf(tableModel.getValueAt(row, 1)));
		tfAuthor.setText((String) tableModel.getValueAt(row, 2));
		tfTitle.setText((String) tableModel.getValueAt(row, 3));
		model.setValue((Date) tableModel.getValueAt(row, 4));
		...
		okButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				tableModel.setValueAt(tfId.getText(), row, 1);
				tableModel.setValueAt(tfAuthor.getText(), row, 2);
				tableModel.setValueAt(tfTitle.getText(), row, 3);
				tableModel.setValueAt(model.getValue(), row, 4);
				tableModel.setValueAt(cbStatus.getSelectedItem(), row, 5);

				// Program.getTableModel().remove(row);
				// Program.getTableModel().add(new Book(Integer.parseInt(tfId.getText()),
				// tfAuthor.getText(), tfTitle.getText(), model.getValue(), (String)
				// cbStatus.getSelectedItem()));

				if (Program.isDatabaseSelected()) {
					int azonosito = (int) tableModel.getValueAt(row, 1);
					String szerzo = (String) tableModel.getValueAt(row, 2);
					String cim = (String) tableModel.getValueAt(row, 3);
					long megjelenes_datuma = ((Date) tableModel.getValueAt(row, 4)).getTime();
					String statusz = (String) tableModel.getValueAt(row, 5);
					// String query = "UPDATE 'konyv' SET 'azonosító'='" + azonosito + "'
					// 'szerzo'='" + szerzo + "' 'cím'='" + cim + "' 'megjelenés dátuma'='" +
					// megjelenes_datuma + "' 'státusz'='" + statusz + "';";
					// String query = "UPDATE konyv SET 'azonosító' = ?, 'szerzo' = ?, 'cím' = ?,
					// 'megjelenés dátuma' = ?, 'státusz' = ?";
