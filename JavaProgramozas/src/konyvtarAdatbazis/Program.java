...
public class Program extends JFrame {
	...
	static JTable table;
	private static BookTableModel tm = new BookTableModel();
	...
	public Program() {
		...
		searchBy.setModel(new DefaultComboBoxModel<String>(new String[] {"azonosítóban", "szerzõben", "címben", "megjelenés dátumában", "státuszban"}));
		...
		btnOpen.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(returnVal == 0) {
					...
					if(path.contains(".db")) Program.setDatabaseSelected(true);
				}
			}
		});
		...
		btnNewEntry.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					newDialog = new NewEntryDialog(Program.this);
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int i = 0; i<tm.getRowCount(); i++) {
					if((Boolean)tm.getValueAt(i, 0)) {
						if(tm.getValueAt(i, 1).getClass().getName().equals("java.lang.String")) {
							id=Integer.parseInt((String)tm.getValueAt(i, 1));
						}else {
							id=((Integer)tm.getValueAt(i, 1));
						}
						tm.remove(id);
					}
				}
		...
		btnEditEntry.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(ticked==1){
					try {
						EditEntryDialog editDialog = new EditEntryDialog(Program.this, row);
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(col == 0) {
					boolean val = (boolean) tm.getValueAt(row, col);
					if(!val) {
						BookTableModel.select(row);
					}else{
						BookTableModel.deSelect(row);
					}
				}
			}
		});
